...
impl<B: KernelBackend> PdafCluster<B> {
    ...
    pub fn reshard(&mut self, seq: u64, prompt: &[u32]) {
        ...
        let idm = self.engine.cfg.dsa.index_head_dim; // k_idx per-token dim
        for pool in &self.cp_pools {
            if let Some(caches) = pool.dsa_caches.get(&seq) {
                for (&fam, cache) in caches.iter() {
                    let t_total = cache.k_idx.len() / idm;
                    for d in 0..n {
    ...
    fn linear_attn_dcp(&mut self, seq: u64, layer_idx: usize, pfx: &str, x: &Tensor, n: usize) -> Result<Tensor> {
        ...
        // per-head gated rmsnorm (checkpoint o_norm is [head_dim])
        ...
        self.engine.backend.gated_rmsnorm(&core_rows, &gate_rows, o_norm_w, self.engine.cfg.rms_norm_eps, &mut normed)?;
    ...
    fn dsa_attn_dcp(&mut self, seq: u64, layer_idx: usize, pfx: &str, x: &Tensor, n: usize) -> Result<Tensor> {
        ...
        let fam = self.engine.cfg
            ...
            .count()
            - 1;
        // projections (same as Engine: indexer queries from the q_lora latent)
        ...
        let mut ki = Tensor::zeros(ki_raw.shape.clone(), DType::F32);
        {
            ...
            for i in 0..n {
                ...
                let inv = 1.0 / (var + self.engine.cfg.rms_norm_eps).sqrt();
                for j in 0..idm {
