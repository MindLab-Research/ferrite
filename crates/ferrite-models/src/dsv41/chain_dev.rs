...
/// 4.7-10.9% divergence's dominant source. The original 40x explosion's root
/// cause is fixed (the ILV interleave loaded the pools for the FUSED batched
/// reader while the e4m3 disable forced the sequential path — plus the s.o
/// zeroing mismatch); `ilv_ok()` now excludes this gate so the pools load
/// NON-interleaved for the sequential path. DEFAULT OFF pending the
/// end-to-end verification; `DSV41_EXPERT_ACT_E4M3=1` arms the official
/// domain.
...
fn hc_tail_split() -> bool {
    ...
    *F.get_or_init(|| std::env::var("DSV41_HC_TAIL_SPLIT").map(|v| v != "0").unwrap_or(true))
    ...
    fn moe(&mut self, layer: usize, ld: &LayerDev) -> Result<()> {
        ...
        // op-level diagnostic: the w2 row 0 of expert 0 (160 bytes as 40 f32)
        // — kind 20 — and its e8m0 scale row (16 bytes as 4 f32) — kind 21.
        // The e4m3's s.o is 40x the e2m1's from comparable dumped activations
        // — the weight norms decide which side's dump mismatches its down's
        // actual input. (Expert 0: the host-side idx is the EMPTY fallback in
        // the normal flow — the routing is device-driven — and the norm
        // statistics are expert-independent.)
        if let Ok(xdp) = std::env::var("DSV41_GT_XDUMP") {
            if layer == 0 && ne >= 2 {
                let e0 = 0i64;
