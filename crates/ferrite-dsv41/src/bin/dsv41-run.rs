...
fn main() -> Result<()> {
    ...
    // End-of-sequence id: generation_config.json -> config.json ->
    // (tokenizer present ? 1 : None). Shared with the unified serve binary
    // (`ferrite-dsv41::serve::resolve_eos`); see that fn for why a single
    // source is not enough for this checkpoint.
    let eos: Option<u32> = ferrite_dsv41::serve::resolve_eos(&dir);
...
// HTTP serve mode
//
// The assembly moved to the LIB (`ferrite_dsv41::serve`): the TP rank pool
// (`TpRankPool`) and the engine builder (`build_serve_engine`) are shared with
// the unified `ferrite-serve --model dsv41` binary, so there is exactly one
// bring-up. The stack itself is the SHARED ferrite-http one (axum routes + SSE
// + the engine driver thread + the tokenizer boundary — the same code GLM
// serves through; see crates/ferrite-http). The chat frame lives with the model
// (`ferrite_dsv41::Dsv41Frame`).
//
// The hand-rolled std::net server this replaced is gone. It also could not have
...
// that the all-reduce's peer stores require. The pool's prologue is the one-shot
...
fn run_serve(
    ...
    // The engine assembly lives in the lib (`ferrite_dsv41::serve`), so the
    // unified `ferrite-serve --model dsv41` binary builds the identical stack:
    // the checkpoint tokenizer + stop set, the TP rank pool, the single-flight
    // adapter. The launcher owns the driver/router/bind/exit.
    let (tok, engine) = ferrite_dsv41::serve::build_serve_engine(dir, so, cfg, tp, eos)?;
