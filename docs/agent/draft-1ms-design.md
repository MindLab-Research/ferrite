...
## 补充：mr 的实际值（决定 markov 切分的真实收益）

`config.json`：`dspark_markov_rank: 256`。⇒ markov 每步读 `vocab × 256 × 4B × 2`（embed+head）= 129280 × 256 × 4 × 2 ≈ **264MB/步** × 5 步 = **1.32GB/步**（不是估算的 126MB——是它的 2 倍，因为 embed 和 head 都要读）。
切分后：1/8 = 33MB/rank/步 × 5 = **165MB/rank/步**——**33MB 恰好贴着 L2（60MB）** ✓ 设计判定仍成立（每步的两次 16.5MB 读都在 L2 内）。
