...
**当前武器（kind-42，314b70b7）**：我们的 logits dump（head 输出的 rank0 切片 [0:16160]，含 EOS 和数数 token）与 ref 的 logits 直接逐位对拍——rel diff 逐步 + EOS/'64' 分数漂移。**⚠️ kind-40/41（层输入 h）的教训**：dump 的 s.h[0:5120] 与 ref 的层 39 输入 h 在两种布局下都不匹配（[hc][dim]=74% vs [dim][hc]=130-138%）——**s.h 的实际布局/语义与假设不符（层 0 的 0.000% 无法验证布局因为初始 h 四块同值）⇒ 逐层 h 对拍需要先读懂 hc_collapse 内核的 s.h 布局**，否则全是伪影。
