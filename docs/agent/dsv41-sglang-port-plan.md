...
## 🎯 expert_fp4_gemm_official 验证结果（2026-09-14 07:50，61e2dfbf）

**数数首次到达 100**（1-51 完美，51 后跳跃到 60→75→80→90→100）：
```
--- generated (149) ---
1 2 3 4 5 ... 49 50 51 60 61 62 63 64 65 66 69 70 71 72 73 74 75 80 81 90 91 92 93 94 95 100
```

**op-level 数据**（注意：pos 15+ 的 RMS 包含 teacher-forcing 发散后的语义差）：
```
pos  0  xn:0.000%(0) kv_RT:0.000%(0) attn_o:0.000%(0) moe_o:1.916%(4673)
pos 15  attn_o:0.618% moe_o:5.865%  ← 包含 51→60 跳跃的语义差
```

**关键解读**：
- pos 0 全部位级（attn_o 0.000%）——prefill 完美
- 1-51 完美数数 = 前 51 个位置 token 与 ref 完全一致
- 51 后跳跃 = 某个位置的 logits 边界翻转（微小数值差导致）
