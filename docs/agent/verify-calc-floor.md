...
| 5 | ~~**head 折叠 + 词表切分**（`VERIFY_HEAD_FOLD=1` + 多行跨卡 argmax，TODO#3）~~<br>→ **已实施：词表切分**（`DSV41_VERIFY_HEAD_SLICED=1` 默认 ON，逐行 `gemv_bf16(n=seg)` + `dsv41_argmax_sliced_rows` 一次 v5 round 批 m 行）；**折叠仍未默认开**，但更正本条：sliced 臂的折叠**不是** v2 核、**没有** K 序差——`DSV41_VERIFY_HEAD_MROWS=1` 走的是 **v1 序** `dsv41_gemv_bf16_v1_mrows`（`chain_dev.rs:3710/10439`，kernel 的 transcription 论证见 `dsv41_glue.cu:461-538`）⇒ **逐位相同、纯发射/内存削减**（993→165MB，6→1 发，−0.10~0.16ms）。有 K 序差的是**旧 v2 折叠** `head_gemv_bf16_mrows`（`gemv_bf16_nt_kernel<NT,1>`，33% echo），它**已无 head 调用点**；`DSV41_VERIFY_HEAD_FOLD` 也被重新指向 v1 核、且只管 unsliced 臂。仍有数值债的是 `DSV41_HEAD_TILELANG=1`（f32 激活 cast bf16，max_rel 2.3e-2） | `chain_dev.rs::verify_head_geom` / `kernels/cuda/dsv41_kernels.cu::dsv41_argmax_sliced_rows` | **1.49 → ~0.19ms（−1.3ms）**；折叠的额外 6×（→0.05ms）**未拿** | **切分：高**（每行与 eager 同一 v1 核，逐位同源；跨卡一次 round）<br>折叠：低（需先证 K 序 parity） | — |
