...
| 4 | `hc_mixes_tail_kernel` | 80 | 12.4 | **0.99** | 10.2% | 0.97 | +0.02 | **未动**（hc-merge 第 14 轮 +3.2ms 回归后 gate OFF）。**2026-09-11 探针实测 = 否决**：整核是 **warp0 串行链**（front 4.9 + sinkhorn 6.5 + collapseP1 0.5 + rmstail 1.2 µs），可藏窗口（collapse P1）只有 0.46µs ≪ sinkhorn 6.5µs ⇒ 重叠收益 0.23µs/次 = **0.018ms/步**（预期 0.2ms 的 1/11）。详见 `/tmp/tail_phase_probe.cu` |
