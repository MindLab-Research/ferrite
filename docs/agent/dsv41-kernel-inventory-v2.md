...
| 4b | `hc_mixes_tail_kernel`（EARLY half，`HC_TAIL_EARLY`） | 80 | **1.7** | — | 关键路径 | — | — | **EARLY = collapse P1（0.5µs）+ rms tail（1.2µs），单 block×1024 线程（1/148 SM）**。⚠️ **不读 `g_hc_part`、不做 ss 归约**（ss/mixes 全在 LATE 分支，`dsv41_kernels.cu:4696` 起）⇒ 对 dots **零数据依赖**。**2026-09-11 终态（B）：EARLY 留在主流**——它只 1.7µs，且唯一消费者是紧随的投影链，主流程序序即完整定序（生产：上一段 hc_post/AR fold 写 `x`=`s.h`/`pre_collapse`；消费：投影链）⇒ **零 event**。曾经的 "EARLY 挪到 side 与 dots 并发"（`e152f47`）需要 `in_ev`+`early_ev` 两条边（否则 side 成图 ROOT、读到陈旧 residual），但 main 仍要等 `early_ev`（等于仍付 1.7µs，只是换成跨流往返），**每 front 却多 4 个图节点** ⇒ 已回退。现发射序列：`main: EARLY -> record(fork_ev)`；`side: wait(fork_ev) -> dots -> LATE -> record(join_ev)`；`in_ev`/`early_ev` 仅存于 ABI（Rust 不再创建，传 null） |
