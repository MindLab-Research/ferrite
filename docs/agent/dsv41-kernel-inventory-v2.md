...
| 4b | `hc_mixes_tail_kernel`（EARLY half，`HC_TAIL_EARLY`） | 80 | **1.7** | — | 关键路径 | — | — | **EARLY = collapse P1（0.5µs）+ rms tail（1.2µs），单 block×1024 线程（1/148 SM）**。⚠️ **不读 `g_hc_part`、不做 ss 归约**（ss/mixes 全在 LATE 分支，`dsv41_kernels.cu:4696` 起）⇒ 对 dots **零数据依赖**。**2026-09-11 hc-early-opt：EARLY 已挪到 side stream 头部与 dots 并发**（dots 4.9µs > EARLY 1.7µs ⇒ 预期全藏 **−0.14ms**，待上机实测）。⚠️ **前提修正**：EARLY 读 `x`(=`s.h`) 与 `pre_collapse`（premix slot），二者都由**主流**在本调用之前写（上一段的 hc_post / AR fold）⇒ EARLY 可以不依赖 dots，但**必须**有一条 main→side 边把它钉在这些生产者之后。原设计"EARLY 不等任何 event"会把 side 的 EARLY 变成图 ROOT（整步 capture 下可先于 hc_post 执行）⇒ 读到**陈旧 residual**。已加 `in_ev`（main 在 dots **前** record、side 在 EARLY 前 wait）补这条边；`fork_ev` 语义不变（dots 后，LATE 等 `g_hc_part`），新增 `early_ev`（side 在 EARLY 后 record，main 在 `hc_front_split` 内 wait）供投影链消费 `out`/`xq`/`xsc` |
...
| **3** | **cross-layer-pipe**：每次 front 拆 ⟨A⟩collapse_norm（留原位）+ ⟨B⟩mixes dots+tail（推迟，与 AR 合并进同一 launch——AR v5 pubred 只有 5 block × **256** 线程（`ferrite_kernels.cu:8428`，已由 5×1024 改小），poll 窗口上百 SM 空闲）| 残差/节点延迟 | **−0.3~0.5**（**复核后下修至 0.1~0.25**，见下）| **Stage B** | 字面"ffn mixes 挪到 L+1"**不可行**：⟨B⟩ 的 `x = s.h` 是 **AR#1 fold 的输出**（`chain_dev.rs:1920` 传 `s.h`；`ferrite_kernels.cu:8598` `ar5_hc_post_col4` 写它），dots+collapse 都吃这个依赖；⟨B⟩ 的 `post/comb` 消费者是**本层** AR#2 的 fold（`chain_dev.rs:2346`）。**唯一跨层物是 `pre` → `premix_slot(2)`**（消费者 = L+1 attn collapse，`chain_dev.rs:2137`），但它与 dots 同批产出。⇒ 可跨层的只剩**层静态权重 staging**（`hc_fn` 1.92MB/站，与残差无关），用 poll 窗口把它预热进 L2，把 dots 的 160KB/block cp.async 从 DRAM 延迟地板（531GB/s = 22GB/s/SM，`dsv41_kernels.cu:4638`）换到 L2。⚠️ AR 核 `step = gridDim.x*blockDim.x`（:8394）会因新增 block 错位 ⇒ 必须按 `blockIdx.x < ar_blocks` 分区；post/comb 单份共享缓冲需双缓冲 |
