...
| 4b | ~~`DSV41_SWIGLU_FOLD`（small-kernel-merge #2）~~ | **已删除（2026-09-12）**：serve A/B 与 quant-fold 合计 **+0.39ms** 回归（代码存在本身又 +0.33ms）⇒ `dsv41_gemm_fp8_mx_swiglu` / `GemvFusion.gu` / `DSV41_SWIGLU_FOLD` gate 全部回滚，回到 `swiglu_limit_q` + `gemm_fp8_mx` 独立 launch | 0（净负） | 已删除，勿再实现（`fork_ev` 是 kernel 级事件，加进 EARLY 的工作必在关键路径）|
...
| P5 | 零 AR 节点（store+stamp 折段末、pubred 折下段首，v5 协议改动）| −0.15 + 40 节点 | **已否决（2026-09-11）**：`DSV41_AR_STAMP_FOLD` serve 29.5s/step；且代码存在性本身 +0.33ms（gate 全 OFF）。代码已删除 |
