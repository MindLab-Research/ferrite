...
| 层 | 假设 | 结果 |
|---|---|---|
| **L-A：M 折叠变免费** | m 行 launch 成本 = m=1 成本（B 列免费，§2.4.1）。现状 mrows = m×单行 ⇒ 该share 直接 ÷6 | mrows 15.2% → 2.5% ⇒ **−12.7 pts ≈ −3.1ms** |
| **L-B：指令坍缩（32×）** | 单行成本本身也从 SIMT 的"160×(20 LDS+13 FP)"降到 mma 的"160×17" 且带深环 | mrows 再 ÷1.5-2 ⇒ 再加 **−0.5~−1.0ms** |
| **L-C：gemv 侧同源** | (b′) 下 m=1/draft 的 gemv 也走 mma；swapAB 对 SIMT 实测 1.76-1.94×（n≥1664）、0.73-0.97×（n≤576，**ks 未调**） | 在 ks 规则修好后按 ≥1.7× 算：15.4% → 9% ⇒ **再加 −1.5ms** |
| **合并去重** | L-A/L-B 与 L-C 覆盖不同 kernel 实例，不重复计 | **中央值 −3.5ms**（verify 24.5 → **21.0ms**） |

### 4.2 区间与风险

| 情形 | 收益 | 触发条件 |
|---|---|---|
| 乐观 | **−4.5ms** | 大 n 两档（wo_b/wq_b）先兑现 + 小 n 两档（wkv/sh）被 ks 规则救活 |
| 中央 | **−3.5ms** | 小 n 档只拿到 1.5×（而非 5×） |
| 保守 | **−2.5ms** | 小 n 档（n=288/512）仍输给 SIMT（复刻 swapAB 的 n≤576 失败）——但 L-A 的 M 折叠收益与 n 无关，兜住下限 |
| 天花板 | ≈ −4.7ms | 投影族压到自身 DRAM 底线（846MB/step ÷ 8TB/s ≈ 0.11ms；实测 1.4TB/s 则 ~0.6ms）⇒ 5.1 → 0.6ms |

**单点重算（846MB 的口径）**：每层投影权重 ≈ wq_a 6.55 + wkv 2.62 + wq_b 5.24 +
wo_b 5.24 + sh 1.47 = **21.1 MB**；40 层 ⇒ **846 MB/step**。现状投影族 5.1ms ⇒ 有效带宽
**166 GB/s（2% of 8TB/s）**。MMA 路的目标带宽 1.4 TB/s ⇒ 0.60ms ⇒ **−4.5ms 天花板**。

**最大不确定性**：**小 n 形状**。`wkv`(512) 与 `sh`(288) 的 tile 数只有 32/18，
**必须**靠 ks=32 把网格顶到 1024/576 warp；如果这条不成立（例如 K 不是 32·ks 的倍数而
ks 掉档），这两档会退回 ~1×，收益落到保守区间。这也是 §3.3 把 ks 规则单列的原因。
