...
nvcc -gencode arch=compute_100a,code=sm_100a -O3 --use_fast_math -std=c++17 \
     -Xptxas -v -c dsv41_kernels.cu -o dsv41_kernels.o
```

**状态**：见 §5.3 的表（本文件交付时由 `-Xptxas -v` 的 `gemm_fp8_mtile_kernel<M>` 行填入）。
验收要点：**0 spill / 0 stack**，regs 与 g2 基线（M=6 时 96）同量级，
`barriers` **从 1 不变**（barrier 数不是负向来源；来源是它的**语义**——它 gate 了多少字节）。

> 若修复后 regs 明显上升（例如 >128 @ M=6），占用会掉到 1 块/SM，那是**新的**负向来源，
> 必须回报而不是继续调 bn。
