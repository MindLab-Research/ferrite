...
**根因（2026-09-09，已定论）**：`bad=60207/61440` 不是片段映射错误 ——
微基准里 token0/1 的前 4 个输出与 SIMT 参考吻合到 **0.3%**，映射是对的。
98% 超 5% 阈值来自 **e4m3 本身只有 3 位尾数（相对误差 3-6%）**：act 一旦量化成 fp8，
每层 MoE down 就引入 ~5% 误差，42 层复利 → 文本崩成 `!`。SIMT 路径读 fp32 act 所以正确。

**结论**：**MoE down 不适合 fp8 MMA**（除非 act 走 bf16 的 m16n8k16，但吞吐只有 fp8 一半、
且权重也要 bf16 翻倍，实测 bf16 SIMT down 与 fp8 同速 11.9µs，没有收益）。
**MoE 的 4ms 必须靠调度/融合解决，不是 MMA。** 这条线索关闭。
