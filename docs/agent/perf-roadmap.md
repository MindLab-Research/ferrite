...
|---|---|---|---|
| **修整步图** | +5-6ms | — | 已修 ⇒ 默认开 |
| MoE gemv 批化（917→~90 启动）| ~2.5ms | **~0.2ms** ✗ | 图内每个节点仍 ~0.2-0.3µs ⇒ 只剩 827 节点的调度差；**实现保留、gated、优先级降到最低** ✓ |
...
**注意**：`scripts/dsv41_profile.sh` 目前固定 `DSV41_GRAPH_STEP=0` ✗ —— 图成为默认后应加一个
"图开/图关"的开关（或直接改成图开，因为两者算子成本相同 ✓ 只有启动/同步口径不同 ✓）。
