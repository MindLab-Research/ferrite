...
> **分析否证（未实测，勿再提案）**：a32 物化的 bf16 LUT —— 见下节。
> **每次改动必须**：看 build error 数 → 同窗口 A/B → 人眼验证文本。

## a32 物化的 bf16 LUT — 分析否证（2026-09-12，未落地）

提案（"final-gap-investigation" top-1）：`s_lut` 由 256×f32 改 256×bf16，声称"一次 LDS.32 取 2 码
⇒ LDS 4→2 per mat4、省 512B smem、~0.1ms"。**三点均不成立，且第一点会破坏数值正确性。**

1. **数值错误（致命）**。`dsv41_a32_mat4`（`dsv41_kernels.cu:89-100`）的四个码是激活里 4 个**任意**字节
   （`b4&0xFF`、`(b4>>8)&0xFF`、`(b4>>16)&0xFF`、`b4>>24`），彼此**无相邻关系**。
   `*(bf162*)(lut + (b4&0xFF))` 取到的是 `LUT[c0]` 与 `LUT[c0+1]`，而 `o.y` 要的是 `LUT[c1]`。
   随机 b4 枚举验证：`o.y`/`o.w` 全错 ⇒ **50% 元素错**（`o.x`/`o.z` 恰好对）。这会直接破坏
   本仓库所有优化都必须满足的**逐位一致性**（fingerprint / 文本双验证）。
2. **LDS 不减半**。gather 的 LDS 条数 = 参与元素数，与表元素宽度无关：要一次 LDS.32 出 2 码，
   必须两码索引相邻（`c` 与 `c^1`），任意码不满足。按 `c>>1` 索引再 shift 选半，只是
   **1 LDS + 1 条额外 ALU 换 1 LDS**；而该 kernel 是**指令数**瓶颈（见下节微基准：unroll 与手写 ILP 全中性），
   故严格更慢。
3. **smem 无收益**。1024→512B 后 gsmem 43392 → 42880B，`232448/42880 = 5.42` ⇒ 仍是 **5 blocks/SM**
   （要到 6 块需 ≤ 38741B）。512B 不跨驻留边界。
...
| ksplit=16 / KStep=128 / NStage=8（**曾是文档里的"最优行"，不是当前默认**） | 5.96-6.57 | ~1300-1400 | 1.76-1.94x |

> ⚠️ **宏默认=ks=8**（`DSV41_SWAPAB_KSPLIT 8`）。上表末行标注为 "ksplit=16"；2026-09-12 复测
> （同一 bench、交叉重复、min of 5）得 ks=16 → **8.32µs**、ks=8 → **7.34µs**、ks=10 → **5.72µs**。
> 即上表的 5.96-6.57 更像 ks=10 的读数；ks 维度已重扫，见下节。
...
#### 环形几何 A/B + ks 扫描 — 2026-09-12（隔离口径实测，结论：环深度不是杠杆）

bench：`/tmp/dsv41_cc20/bench_ring.cu`（`#include "dsv41_kernels.cu"`，几何用 `-DDSV41_SWAPAB_*`
覆盖，两次调用间复用同一权重 buffer）。口径：n=1664 k=5120、每配置 300 call、CUDA events、
**交叉重复 3-5 轮取 min**（min≈avg，方差 <1%）。"cold" = 每次调用前 memset 512MB 冲 L2，单独测
memset 耗时并**扣除**。B300 单卡独占。

| KStep | NStage | ks | hot µs/call | cold µs/call |
|---|---|---|---|---|
| 128 | 8 | 8（**当前默认**） | 7.31-7.37 | 9.98 |
| 64 | 16 | 8（"环满"假设的提案） | **8.66-8.72** | **10.64** |
| 64 | 11 | 8（nk=10 == NSTAGE-1，**恰好填满**） | **8.43** | - |
| 128 | 16 | 8 | 7.75-8.34 | - |
| 32 | 16 | 8 | 10.45 | - |
| 64 | 8 | 16 | 6.28-6.29 | 8.55 |
| **128** | **8** | **10** | **5.72** | **8.36** |
| 128 | 8 | 16 | 8.32-8.34 | 10.37 |
| 128 | 8 | 4 / 5 / 20 | 8.79 / 8.06 / 8.07 | - |

**被否证的假设**（v19 中性 = "kc=640 → nk=5 < NSTAGE-1=7 → 环填不满 → 细化 stage 即可把
8x warps 变成 8x in-flight"）：**恰好填满**的 KStep=64/NStage=11（nk=10 == NSTAGE-1）实测
**8.43µs**，比"填不满"的默认 7.31µs **更慢**；KStep=64/NStage=16 同样更慢（8.66 vs 7.31），
冷口径亦然（10.64 vs 9.98）。两个直接原因：
1. **覆盖 DRAM 往返的是"每 warp 在飞字节数" = NSTAGE × KStep × 16**，该乘积近似守恒
   （8×128 ≈ 16×64）——细化 stage 只把同样的字节拆成更多次、每次更小，并未增加在飞量；
2. **MLP 实际来自每 SM 上并发的 one-warp block 数**（smem 受限，128/8 时 19236B/block →
   ~11 block/SM），不是单个 warp 的环深度。细化 stage 反而（a）每字节的
   wait/commit/`__syncwarp`/空 stage 开销翻倍，（b）16B 行填充占比从 11%（144B 行）升到 20%（80B 行），
   （c）环 smem 变大→驻留 block 变少。

**真正的杠杆是 `ks`（warp 数 × kc），且在 k=5120 上非单调**：ks=4 → 8.79、5 → 8.06、8 → 7.34、
**10 → 5.72（最优）**、16 → 8.32、20 → 8.07。ks=10 只是"在 k=5120（160 个 k-block）上整除且落在
最优带"。⚠️ **launcher 到不了 ks=10**：`ks = kSwapabKSplit` 后只按 2 折半（8→4→2→1），
要么把宏改成 10（但 k=2048 时 64 % 10 ≠ 0 → 折半到 **ks=2**，是回归），要么把 launcher 的
"从宏值折半"改成"取 ≤ cap 的最大合法 ks"——后者需要逐形状复测 + serve A/B 才可翻默认。
