...
- ✅ **2026-09-11 机制复用（生产路径，默认 ON，与 P1d 无关地独立开关）**：`dots+LATE` 合并 = `hc_dots_late_kernel`（`dsv41_kernels.cu:6632`），grid=`(mix, rows)`、block=`DSV41_HC_DOTS_T`(128)、`atomicAdd` 选举**最后一个 publish 的 dot 块**跑 LATE 半，gate `DSV41_HC_DL_MERGE`（默认 ON，`=0` 回两 launch A/B 臂）。**刻意只取选举机制、不取 P1d 的形状**：① `split=1`（无 ck 合并 ⇒ 逐位等价）；② **无 collapse block**（EARLY 仍是侧流头部的独立 launch）；③ tail 走 **`ss_in==1`**（用 dots 的 ss 分块，不重读整行）——P1d 是把这条优化丢了；④ `block=128` 而非 1024（点积并行度与两 launch 版相同，也不占 1024 线程 SM slot）。⇒ 上面「P1d 回归归因」三条（整行重读 20480 float / ck 循环 8× L2 / 在整格 drain 期间跑串行 tail）**都不适用**；收益 = 每 front 省 1 个图节点（2/层 × 61 层），代价为 0 新增并发。⚠️ 待上机 A/B（`DSV41_HC_DL_MERGE=0` 臂）确认。|
