...
|---|---|---|---|---|---|---|---|
| **1** | ~~down-vec-320~~ **已定案回退** | `expert_gemv_fp4_down_reduce_kernel`（现 :1408），`vec==3` 4 值形式 | ~~−0.15~0.20~~ **实测 +0.31 回归→已回退** | — | — | `01291b2` 的 nv8 宽加载已移除；现行默认 `DSV41_DOWN_VEC4`（ON）→ `g_down_fp4_mode==3`（4 值 uint16），`DSV41_DOWN_VEC4=0` 回退 `g_expert_fp4_mode`（=2） | 复核见 `dsv41-kernel-inventory-v3.md` §(B)：nv8 是寄存器/占用率回归；当前 HEAD 无 `nv8` 标识符 |
