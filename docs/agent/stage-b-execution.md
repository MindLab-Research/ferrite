...
| **3** | **quant 生产者直出 T2** | `dsv41_kernels.cu:86` kernel、launcher `:1625`；T1 已做 `chain_dev.rs:700/1443/1544` | **−0.13~0.28** | 中 0.5 | **0.103** | 剩余 ~126 次/步，可摘的 80 次集中在 4 个 site：`:1995`(wo_a)、`:2043`(wo_b)、`:2701`(shared w1/w3)、`:710`(lin)。给这些生产者加 fp8 epilogue（absmax 需整行 ⇒ 复用 T1 的 `is_last`/pointer-gated 机制） | 每 site 一个 env（如 `DSV41_QUANT_EPI_WOA/WOB/SH`）；`=0` 回落老 `quant1`。位级：scale 是同一 absmax 的同一舍入 ⇒ 应逐位；不逐位则立刻回退该 site |
