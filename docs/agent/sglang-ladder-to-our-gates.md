...
## 🔴 `DSV41_VERIFY_FORK=1` 损坏输出（不许开！）——2026-09-13 19:12 字节级实证

**证据**（`~/num100_last.txt` 的 `cat -A`，**不是**被截断的打印 ✗）：

```
1$ 2$ 3$ 44$ 5$ 6$ 7$ 8$ 88$ 99$ 10$ 1111$ 12$ 13$ 1414$ 15$ 16$ 1717$ 18$ 19$ 2020$ 21$ 22$ 2323$ 2424$ 2525$ 2626$ ...
```

- 该文件 mtime **19:11:33** ⇒ 对应**带 `DSV41_VERIFY_FORK=1` 的那次运行**；而更早**不带 fork** 的那份是**干净的 `1..12`** ✓
  ⇒ **⇒ 是 fork 造成的** ✓。
- 形态：**数字被重复**（`44/88/99/1111/1414/1717/2020/2323/2424/…`）⇒ 典型**流/缓冲竞争**征状 ✗
  ⇒ 与"把 side-stream fork 搬进 verify"的设计意图（逐位无风险）**不符** ✗ ⇒ 该实现有竞态 ✓。
- ⇒ **纪律：`VERIFY_FORK` 一律保持 OFF ✗**；任何含它的臂（无论测什么）**数都会被污染** ⇒ 必须重跑 ✓。

**（附）这一步也顺带证明了"看字节级原文"的必要性**：同一份输出在**任务输出层**会被截断/串行化 ✗，
只有 `cat -A ~/num100_last.txt` 才能看到真相 ✓ ⇒ **判文本红线一律用这个 ✓。**
