...
| 6 | **compress side** | kv-source 层（2/8/14/20）的 4 个 compress launch（~30µs）→ **3 个**（COMPRESS_FUSE） | `lin2` 之后（与 dual_chain fork 同点） | `window_idxs` 之后、**indexer 之前**（indexer 读本层 `latent` 与 `clen`） | `DSV41_COMPRESS_SIDE`（`chain_dev.rs:364`）；需**新开 `side_stream3`**（与 kv 链时间窗完全重叠）；**`DSV41_COMPRESS_FUSE`（`chain_dev.rs:382`，默认 ON）**：decode 下 `state`+`pool`+`commit` 三个 1-block launch 合一为 `dsv41_compressor_fused`（`dsv41_kernels.cu:2638`/`:6285`），省 2 launch × 3 层 |
| 7 | **dots+LATE 合并**（hc-dl-merge，2026-09-11） | 不是拆链，而是把 side 链的 dots(4.9µs)+LATE(10.7µs) **两个节点合成一个**：`hc_dots_late_kernel`（`dsv41_kernels.cu:7867`）用 `atomicAdd` 选举**最后一个 publish 的 dot 块**跑 LATE 半（`hc_pre_persist_mb_kernel` 的无自旋模式） | —（side 链内部） | 不变（`hc_tail_join` wait `join_ev`） | `DSV41_HC_DL_MERGE`（默认 ON，`=0` 回两 launch）；EARLY 保持侧流头部独立 launch（EARLY+LATE 合并已证不可行：main 等待 1.7µs→17µs）；逐位等价（split=1 + tail 走 `ss_in==1`）|
