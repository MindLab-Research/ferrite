...
| 30 | `quant_fp4`（激活）| :1783 | rows=1, cols=5120, block=32。**已融合**：`DSV41_QUANT_FP4_FUSE`（默认 ON）时 `dsv41_quant_fp4` 发射单核 `quant_fp4_fused_kernel`（量化+打包一步，bit-exact），旧 `quant_kernel<1>` + `fp4_pack_kernel` 两段路径保留为回退（env=0，或 block 奇数/>256）。~~再融合（`DSV41_QUANT_FOLD`）：2026-09-12 **已删除**（净负 +0.39ms），见下文~~ |
...
- ~~**QUANT_FOLD `DSV41_QUANT_FOLD`（实现 2026-09-11 → 2026-09-12 已删除）**~~ ✗：
  **结论**：v12/v13 serve A/B 实测回归（与 swiglu-fold 合计 **+0.39ms**），且两项 fold 的**代码存在本身**又加 +0.33ms 未归因回归。**机制（已定案）**：`fork_ev` 是 **kernel 级**事件——EARLY 整核跑完才 record，而主流的投影组就 wait 在这个 event 上 ⇒ 加进 EARLY 的**任何**工作都直接落在关键路径（fp4 直出 ≈1.8µs × 80 front = +0.14ms，换回的 quant launch 只有 −0.072ms）。
  **已整体回滚**（ABI 回到 `006bd0c^` pre-fold）：`hc_mixes_tail_kernel` / `dsv41_hc_front` / `dsv41_hc_front_split` 去掉 `xq4`/`xsc4` 形参，EARLY epilogue 的 fp4 直出、`chain_dev.rs` 的 `quant_fold()` / `s.xq4_of_xn_valid` / `moe()` 的 skip 分支、`device.rs` 的 FFI 参数与 wrapper 全部移除。ffn 侧恢复「独立 `quant_fp4(s.xn)` launch」的 pre-fold 行为。
  ⚠️ 同批删除的还有 **SWIGLU_FOLD**（`dsv41_gemm_fp8_mx_swiglu`，共享专家 w2 的 prologue 现算）：两者一起 +0.39ms。保留 A4 `DSV41_SWIGLU_Q` + A5 `DSV41_MOE_EPI_ADD`。
