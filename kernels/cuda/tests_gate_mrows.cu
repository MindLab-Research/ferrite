...
// ROW-FOLD (`Device::gemv_bf16_mrows` -> `ferrite_gemv_bf16_nt`), the
// m-activation-row form of the single-row gate GEMV.
//
...
// fold replaces the m calls with ONE `ferrite_gemv_bf16_nt(..., nrows = m)`. The
// parity claim is
//
//     ONE nt call (nrows = m)  ==  m v2 calls (nrows = 1), BIT FOR BIT,
//
...
// nt entry, which carries v2's exact program including the fold.
//
...
void case_gate(int m, int n, int k) {
    ...
    float *dx = nullptr, *out_a = nullptr, *out_b = nullptr;
    ...
    // ---- arm B: ONE m-row nt launch (the fold) ----
    ...
    if (eq && covered) {
        ...
        ++g_fails;
        printf("  FAIL m=%d n=%d k=%d (WPR=%d) eq=%d covered=%d\n", m, n, k, n >= 1024 ? 4 : 8,
               (int)eq, (int)covered);
        int shown = 0;
        for (size_t i = 0; i < on && shown < 8; ++i) {
            ...
            memcpy(&ub, &b[i], 4);
            if (ua != ub) {
                printf("       i=%zu (row %zu col %zu) v2=%g/%08x nt=%g/%08x\n", i, i / n, i % n,
                       a[i], ua, b[i], ub);
                ++shown;
            }
        }
...
int main() {
    printf("dsv41 gemv_bf16_nt row-fold self-test (nt(nrows=m) == m x v2(nrows=1))\n");
