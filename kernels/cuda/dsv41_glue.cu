...
__global__ void compress_commit_kernel(const float* __restrict__ latent,
    ...
    for (int c = threadIdx.x; c < hd; c += blockDim.x) {
        ...
        // VERBATIM with `compressor_fused_kernel`'s stage 3 (dsv41_kernels.cu) -
        // keep the two bodies in sync. fp4 (e2m1) with ONE float8_e4m3 scale per
        // 16 elements, AFTER the rope and BEFORE the cache write
        // (model.py:758-761); the scale is `e4m3(amax/6)` with amax floored at
        // 6 * 2^-9 (kernel.py:129-184, the `scale_dtype == FP8` branch), NOT the
        // indexer's power-of-two block-32 form.
        {
