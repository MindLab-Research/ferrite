...
**已验证无效（gate off 保留代码）**：e4m3 MMA down v1/v2（**修正基准参数后**——旧 bench 误用 inter_shared=512，SIMT 对 klen≠256 走标量慢路径，造出"108µs/指令瓶颈/2.14x"三重假象；正确参数下 SIMT 43.7µs=in-serve 实测，v1 48.6 / v2 连续读 49.4 均**更慢**——down 在 2.5TB/s 有效带宽的地板，勿再试 MMA 化）；n==16 bf16 wmma 投影（无 K-split，6x 回退）；P2P AR 复测仍死锁（30s 监护杀，驱动未 wedge）；kpool grid cap（15.29 回退——空块不是成本，全 grid 的内存级并行才是）；**bf16 cast 缓存（FERRITE_XB_CACHE=1，3 个变体全崩）**：v1 池化缓冲扰动 batch 池地址稳定性契约（图内嵌中间缓冲地址被推移→36tok/1 Xid）；v2 immortal 缓冲+capture-only 仍崩（**capture 期 alloc_immortal = cudaMalloc inside capture = 非法**，xq_cached 同款雷）；v3 ptr-only key+dry 预注册仍崩（err 901 "previous error during capture"——前序 op 失败未定位，4 轮后关停）。**教训：隔离基准必须用生产的真实 shape（inter_shared 等），错一个参数结论全反；capture 期间任何分配都是雷；batch 池的分配确定性对跨 retire 保留的图是承重的。**
